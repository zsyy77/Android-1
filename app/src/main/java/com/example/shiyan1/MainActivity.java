...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    Button btn_clr,btn_del,btn_eq;//C,<-,=
    Button btn_sqrt;//平方根
    EditText et_input;
    ...
    public void onClick(View v) {
        String str=et_input.getText().toString();//获取输入框字符，存储在str
        int id = v.getId();//获取点击按钮
        ...
        } else if (id == R.id.btn_sqrt ||id == R.id.btn_add || id == R.id.btn_sub || id == R.id.btn_mul || id == R.id.btn_div) {
            if (clr_flag) {
                clr_flag = false;
                str = "";
                et_input.setText("");
    ...
    private void getResult() {
        String exp = et_input.getText().toString();//获取输入框里的字符串
        ...
        if (!s1.equals("") && !s2.equals("")) {
            ...
            if (op.equals("÷")) {
                Toast toast;
                 toast=Toast.makeText(this,"除数不能为0",Toast.LENGTH_SHORT);
                if (d2 == 0)
                    toast.show();
                else cnt = d1 / d2;
            }
        ...
        //如果s1是空    s2不是空  就执行下一步
            ...
            if(op.equals("√")){
                ...
                if(d2<0){
                    Toast toast;
                    toast=Toast.makeText(this,"d2不能为负数",Toast.LENGTH_SHORT);
                    toast.show();
                }
            }
            if (!s2.contains(".")) {
